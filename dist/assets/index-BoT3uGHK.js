(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))s(r);new MutationObserver(r=>{for(const o of r)if(o.type==="childList")for(const a of o.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&s(a)}).observe(document,{childList:!0,subtree:!0});function n(r){const o={};return r.integrity&&(o.integrity=r.integrity),r.referrerPolicy&&(o.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?o.credentials="include":r.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function s(r){if(r.ep)return;r.ep=!0;const o=n(r);fetch(r.href,o)}})();const ge="modulepreload",pe=function(e){return"/"+e},G={},$=function(t,n,s){let r=Promise.resolve();if(n&&n.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),i=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(n.map(l=>{if(l=pe(l),l in G)return;G[l]=!0;const d=l.endsWith(".css"),v=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${v}`))return;const m=document.createElement("link");if(m.rel=d?"stylesheet":ge,d||(m.as="script"),m.crossOrigin="",m.href=l,i&&m.setAttribute("nonce",i),document.head.appendChild(m),d)return new Promise((x,f)=>{m.addEventListener("load",x),m.addEventListener("error",()=>f(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(a){const i=new Event("vite:preloadError",{cancelable:!0});if(i.payload=a,window.dispatchEvent(i),!i.defaultPrevented)throw a}return r.then(a=>{for(const i of a||[])i.status==="rejected"&&o(i.reason);return t().catch(o)})};function c(e,t="info"){document.querySelectorAll(".toast").forEach(o=>o.remove());const s=document.createElement("div"),r={success:"#25D366",error:"#ef4444",info:"#8696a0",warning:"#f59e0b"};s.className="toast fixed right-4 top-4 p-4 rounded-lg text-white shadow-lg transform translate-x-full transition-all duration-300 z-50",s.style.backgroundColor=r[t],s.innerHTML=`
  ...
  `,document.body.appendChild(s),setTimeout(()=>{s.style.transform="translateX(0)"},100),setTimeout(()=>{s.style.transform="translateX(100%)",setTimeout(()=>s.remove(),300)},3e3)}async function he(){try{return await Notification.requestPermission()==="granted"}catch(e){return console.error("Erreur permissions notifications:",e),!1}}const S="https://mon-serveur-cub8.onrender.com";async function ye(){try{const e=we();if(!e)return console.error("Aucun utilisateur connecté"),[];console.log(`Récupération chats pour ${e.name} (ID: ${e.id})`);const t=await fetch(`${S}/chats`);if(!t.ok)throw new Error("Erreur réseau");const n=await t.json();console.log("Total chats dans la base:",n.length);const s=n.filter(r=>r.ownerId===e.id);return console.log(`Chats de ${e.name}:`,s.length),s}catch(e){return console.error(" Erreur getChats:",e),[]}}async function ee(e){try{console.log("Récupération messages pour chat:",e);const t=await fetch(`${S}/chats/${e}`);if(!t.ok)throw new Error("Erreur réseau");return(await t.json()).messages||[]}catch(t){return console.error("Erreur getMessages:",t),[]}}async function ve(e,t){try{console.log(` Mise à jour chat ${e}:`,t);const n=await fetch(`${S}/chats/${e}`);if(!n.ok)return console.warn(` Chat ${e} non trouvé pour mise à jour`),null;const s=await n.json();Object.assign(s,t);const r=await fetch(`${S}/chats/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!r.ok)throw new Error("Erreur mise à jour");return console.log(`Chat ${e} mis à jour`),await r.json()}catch(n){return console.error(" Erreur updateChat:",n),null}}async function xe(e,t){try{console.log(`Mise à jour statut utilisateur ${e}:`,t?"en ligne":"hors ligne");const n=await fetch(`${S}/users/${e}`);if(n.ok){const r=await n.json();r.isOnline=t,r.lastSeen=new Date().toISOString(),await fetch(`${S}/users/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)})}const s=await fetch(`${S}/chats`);if(s.ok){const o=(await s.json()).filter(a=>a.contactId===e);for(const a of o)a.isOnline=t,a.lastSeen=new Date().toISOString(),a.status=t?"en ligne":"hors ligne",await fetch(`${S}/chats/${a.id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})}console.log(` Statut utilisateur ${e} mis à jour`)}catch(n){console.error(" Erreur updateUserStatus:",n)}}function we(){const e=localStorage.getItem("currentUser");return e?JSON.parse(e):null}async function be(){try{console.log(" Initialisation de tous les chats croisés...");const t=await(await fetch(`${S}/users`)).json();console.log(` ${t.length} utilisateurs trouvés`);const s=await(await fetch(`${S}/chats`)).json();let r=0;for(const o of t)for(const a of t)if(o.id!==a.id&&!s.find(l=>l.ownerId===o.id&&l.contactId===a.id)){const l={id:`${o.id}_${a.id}_${Date.now()+Math.random()}`,ownerId:o.id,contactId:a.id,name:a.name,phone:a.phone,avatar:a.avatar,status:"Hors ligne",isOnline:a.isOnline||!1,lastSeen:a.lastSeen||new Date().toISOString(),unread:0,time:new Date().toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"}),lastMessage:"",lastMessageTime:new Date().toISOString(),messages:[]};await fetch(`${S}/chats`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)}),r++,console.log(` Chat créé: ${o.name} -> ${a.name}`)}return console.log(` Initialisation terminée: ${r} chats créés`),{success:!0,chatsCreated:r}}catch(e){throw console.error("Erreur initialisation chats:",e),e}}let te=null;function Ee(){const e=localStorage.getItem("currentUser");if(e)try{const t=JSON.parse(e);return ne(t),t}catch(t){console.error("Erreur parsing user:",t),localStorage.removeItem("currentUser")}return null}function b(){return te||Ee()}function ne(e){te=e,e?(localStorage.setItem("currentUser",JSON.stringify(e)),xe(e.id,"en ligne").catch(console.error)):localStorage.removeItem("currentUser")}function se(){window.refreshInterval&&clearInterval(window.refreshInterval),localStorage.removeItem("currentUser"),window.location.reload()}async function $e(e,t){try{if(!e&&!t)return c(" Veuillez remplir tous les champs","error"),null;if(!e)return c(" Le nom est obligatoire","error"),null;if(!t)return c(" Le numéro de téléphone est obligatoire","error"),null;if(e.length<2)return c(" Le nom doit contenir au moins 2 caractères","error"),null;if(e.length>50)return c(" Le nom ne peut pas dépasser 50 caractères","error"),null;if(!/^\d+$/.test(t))return c(" Le numéro ne doit contenir que des chiffres","error"),null;if(t.length!==9)return c(" Le numéro doit contenir exactement 9 chiffres","error"),null;if(!t.startsWith("7"))return c(" Le numéro doit commencer par 7 (format sénégalais)","error"),null;const n=await fetch("https://mon-serveur-cub8.onrender.com/users");if(!n.ok)return c(" Erreur de connexion au serveur","error"),null;const s=await n.json(),r=s.find(o=>o.name.toLowerCase().trim()===e.toLowerCase().trim()&&o.phone.trim()===t.trim());if(r)return ne(r),c(` Bienvenue ${r.name} !`,"success"),r;{const o=s.find(i=>i.name.toLowerCase().trim()===e.toLowerCase().trim()),a=s.find(i=>i.phone.trim()===t.trim());return c(o&&!a?" Ce nom existe mais avec un autre numéro de téléphone":!o&&a?"Ce numéro existe mais avec un autre nom":"Aucun compte trouvé avec ces informations","error"),null}}catch(n){return console.error("Erreur de connexion:",n),c(" Erreur de connexion au serveur. Vérifiez votre connexion internet.","error"),null}}function Se(e){const t=document.createElement("div");t.className="min-h-screen flex items-center justify-center bg-[#111b21] px-4",t.innerHTML=`
  ...
  `,document.body.appendChild(n),o&&K(r.id,o.id),n.querySelector("#closeViewer").addEventListener("click",()=>{document.body.removeChild(n)}),n.querySelector("#likeBtn").addEventListener("click",async()=>{if(o){const f=n.querySelector("#likeBtn");f.style.transform="scale(1.5)",f.style.transition="transform 0.2s ease",a(f),setTimeout(()=>{f.style.transform="scale(1.1)"},200),console.log(` Clic sur like pour story ${r.id}`);const p=await Oe(r.id,o.id);p&&(r=p,e[s]=p,i(),l())}else c(" Connectez-vous pour liker les stories","error")});function a(f){for(let p=0;p<5;p++){const g=document.createElement("div");g.innerHTML="❤️",g.style.position="absolute",g.style.fontSize="20px",g.style.pointerEvents="none",g.style.zIndex="1000";const E=f.getBoundingClientRect();g.style.left=E.left+Math.random()*E.width+"px",g.style.top=E.top+"px",document.body.appendChild(g),g.animate([{transform:"translateY(0px) scale(1)",opacity:1},{transform:"translateY(-50px) scale(0.5)",opacity:0}],{duration:1e3,easing:"ease-out"}).onfinish=()=>g.remove()}}function i(){const f=n.querySelector("#likeBtn"),p=r.likes.some(g=>g.userId===(o==null?void 0:o.id));f.className=`like-button flex items-center space-x-2 transition-all duration-200 transform hover:scale-110 ${p?"text-red-500 scale-110":"text-white hover:text-red-300"}`,f.querySelector("span").textContent=r.likes.length,console.log(` Bouton like mis à jour: ${r.likes.length} likes, isLiked: ${p}`)}function l(){const f=n.querySelector(".absolute.bottom-0 .flex.items-center.justify-between .flex.items-center.space-x-2");r.isMonetized?f.innerHTML=`
        <div class="bg-green-500 px-3 py-1 rounded-full text-sm animate-pulse flex items-center space-x-1">
          <i class="fas fa-coins text-yellow-300"></i>
          <span class="font-bold">${r.earnings} FCFA</span>
        </div>
    ...
    `:""}function X(e){const t=new Date,n=new Date(e),s=Math.floor((t-n)/(1e3*60*60));return s<1?`il y a ${Math.floor((t-n)/6e4)}m`:`il y a ${s}h`}let B=[],u=null,L=null,C="chats",_=!1,D=!1;window.showSimpleGroups=null;window.currentChat=null;window.currentGroup=null;document.addEventListener("DOMContentLoaded",()=>{console.log(" WhatsApp Web démarré"),ze()});async function ze(){const e=document.getElementById("mainContainer"),t=document.getElementById("loginContainer"),n=b();n?(console.log(" Utilisateur connecté:",n.name),r()):(console.log(" Aucun utilisateur connecté"),s());function s(){e.style.display="none",t.style.display="block",t.innerHTML="";const o=Se(a=>{console.log(" Connexion réussie pour:",a.name),r()});t.appendChild(o)}function r(){t.style.display="none",e.style.display="flex",Fe()}}async function Fe(){try{console.log(" Initialisation de l'interface..."),await I(),He(),it(),ue(),Ce((e,t)=>{console.log(" Nouveau message reçu:",e),dt(e,t)},(e,t)=>{console.log(` Statut utilisateur ${e}:`,t?"en ligne":"hors ligne"),ut(e,t)}),lt(),await he(),console.log(" Interface principale initialisée")}catch(e){console.error(" Erreur initialisation:",e),c("Erreur de chargement","error")}}async function I(){try{if(console.log(" Chargement des chats..."),B=await ye(),console.log(`${B.length} chats chargés`),C==="chats"&&z(),u){const e=await ee(u.id);ce(e)}}catch(e){console.error(" Erreur chargement chats:",e),c("Impossible de charger les conversations","error")}}function He(){console.log(" Configuration des événements...");const e=document.getElementById("userAvatarButton");e&&e.addEventListener("click",ot);const t=document.getElementById("backToChats");t&&t.addEventListener("click",de);const n=document.getElementById("logoutButton");n&&n.addEventListener("click",se);const s=document.getElementById("backButton");s&&s.addEventListener("click",rt),st(),We(),Ve(),Je(),Ge(),window.addEventListener("resize",ct)}function Ge(){const e=document.getElementById("newChatBtn");e&&e.addEventListener("click",()=>{if(!b()){c(" Erreur: utilisateur non connecté","error");return}U(async n=>{console.log(" Contact ajouté:",n.name),await I(),c(`${n.name} ajouté avec succès`,"success")})})}function Ve(){const e=document.getElementById("searchInput");e&&e.addEventListener("input",t=>{const n=t.target.value.toLowerCase().trim();Xe(n)})}function Je(){const e=document.querySelectorAll(".filter-tab");e.forEach(t=>{t.addEventListener("click",()=>{e.forEach(s=>{s.classList.remove("active","bg-green-600","text-white"),s.classList.add("text-gray-400")}),t.classList.add("active","bg-green-600","text-white"),t.classList.remove("text-gray-400");const n=t.dataset.filter;n==="all"||!n?(C="chats",z()):Ye(n)})})}function We(){const e=document.querySelectorAll(".nav-item");e.forEach(t=>{t.addEventListener("click",async n=>{n.preventDefault(),n.stopPropagation();const s=t.dataset.view;if(console.log(" Navigation vers:",s),!(_||C===s)){_=!0;try{switch(C=s,e.forEach(r=>r.classList.remove("active")),t.classList.add("active"),s){case"chats":await Y();break;case"status":await Ze();break;case"communities":c(" Groupes - Fonctionnalité en développement","info");break;case"settings":Ke();break}console.log("Navigation terminée vers:",s)}catch(r){console.error(" Erreur navigation:",r),C="chats",await Y()}finally{setTimeout(()=>{_=!1},500)}}})})}async function Y(){console.log(" Affichage vue chats");const e=document.getElementById("sidebar"),t=document.getElementById("profilePanel");t.style.display="none",e.style.display="flex",await I()}async function Ze(){console.log(" Affichage vue stories");const e=document.getElementById("sidebar"),t=document.getElementById("profilePanel");t.style.display="none",e.style.display="flex",await ae()}function Ke(){console.log(" Affichage vue paramètres");const e=document.getElementById("sidebar"),t=document.getElementById("profilePanel");t.style.display="none",e.style.display="flex",Qe()}async function ae(){const e=document.getElementById("chatList"),t=b();if(!(!e||!t))try{const n=await Ne();e.innerHTML=`
      <div class="p-4">
            ...
            <div class="story-item p-3 hover:bg-[#202c33] rounded-lg cursor-pointer transition-colors" data-story-id="${r.id}">
              <div class="flex items-center space-x-3">
                ...
                <div class="flex-1">
                  ...
                  <div class="text-gray-400 text-sm">${mt(r.timestamp)}</div>
                </div>
    ...
    `;const s=document.getElementById("createStoryBtn");s&&s.addEventListener("click",()=>{qe(async r=>{C==="status"&&await ae()})}),document.querySelectorAll(".story-item").forEach(r=>{r.addEventListener("click",()=>{const o=r.dataset.storyId,a=n.findIndex(i=>i.id===o);a!==-1&&Ue(n,a)})})}catch(n){console.error("Erreur chargement stories:",n),e.innerHTML=`
  ...
  `;const n=document.getElementById("addContactBtn");n&&n.addEventListener("click",()=>{U(async a=>{await I(),c(`${a.name} ajouté avec succès`,"success")})});const s=document.getElementById("initChatsBtn");s&&s.addEventListener("click",async()=>{try{c(" Initialisation de tous les chats...","info");const a=await be();c(`${a.chatsCreated} chats créés !`,"success"),await I()}catch(a){console.error(" Erreur initialisation:",a),c(" Erreur lors de l'initialisation","error")}});const r=document.getElementById("refreshBtn");r&&r.addEventListener("click",async()=>{c(" Actualisation...","info"),await I(),Be(),c(" Actualisé !","success")});const o=document.getElementById("logoutBtn");o&&o.addEventListener("click",se)}function Xe(e){document.querySelectorAll(".chat-item").forEach(n=>{var o,a;const s=((o=n.querySelector(".chat-name"))==null?void 0:o.textContent.toLowerCase())||"",r=((a=n.querySelector(".chat-message"))==null?void 0:a.textContent.toLowerCase())||"";s.includes(e)||r.includes(e)?n.style.display="block":n.style.display="none"})}function Ye(e){let t=[...B];switch(e){case"unread":t=t.filter(n=>n.unread>0);break;case"favorites":t=t.filter(n=>n.isFavorite);break;case"groups":t=t.filter(n=>n.isGroup);break}et(t)}function et(e){const t=document.getElementById("chatList");t&&(t.innerHTML="",e.forEach(n=>{const s=ie(n);t.appendChild(s)}))}function z(){const e=document.getElementById("chatList");if(!e)return;const t=b();if(!t)return;if(console.log(` Rendu de ${B.length} chats pour ${t.name}`),e.innerHTML="",B.length===0){e.innerHTML=`
      <div class="text-center py-8 text-gray-400">
    ...
    `;const s=document.getElementById("addFirstContact");s&&s.addEventListener("click",()=>{U(async r=>{await I(),c(`${r.name} ajouté avec succès`,"success")})});return}[...B].sort((s,r)=>{const o=new Date(s.lastMessageTime||s.time);return new Date(r.lastMessageTime||r.time)-o}).forEach(s=>{const r=ie(s);e.appendChild(r)})}function ie(e){const t=document.createElement("div");t.className="chat-item px-4 py-3 cursor-pointer hover:bg-[#202c33] transition-colors border-b border-gray-700",t.dataset.chatId=e.id;const n=e.unread>0,s=e.isOnline;return t.innerHTML=`
    <div class="flex items-center space-x-3">
      <div class="relative">
  ...
  `,t.addEventListener("click",()=>tt(e.id)),t}async function tt(e){var t;try{if(console.log(" Ouverture chat:",e),de(),u=B.find(n=>n.id===e),window.currentChat=u,!u){console.error(" Chat non trouvé:",e);return}console.log("Chat ouvert:",u.name),u.unread>0&&(u.unread=0,await ve(u.id,{unread:0})),document.querySelectorAll(".chat-item").forEach(n=>{n.classList.remove("bg-[#202c33]")}),(t=document.querySelector(`[data-chat-id="${e}"]`))==null||t.classList.add("bg-[#202c33]"),N()&&(document.getElementById("sidebar").style.display="none"),document.getElementById("chatArea").style.display="flex",nt(),await ce(),le(),C==="chats"&&z()}catch(n){console.error(" Erreur ouverture chat:",n),c("Erreur lors de l'ouverture du chat","error")}}function nt(){const e=document.getElementById("chatHeader"),t=document.getElementById("chatAvatar"),n=document.getElementById("chatName"),s=document.getElementById("chatStatus");if(e&&u){e.style.display="flex",t.innerHTML=`<img src="${u.avatar}" alt="${u.name}" class="w-10 h-10 rounded-full object-cover">`,n.textContent=u.name,s.textContent=u.isOnline?"en ligne":u.status;const r=document.getElementById("callButtons");if(r){r.innerHTML=`
      ...
      `;const o=document.getElementById("audioCallBtn"),a=document.getElementById("videoCallBtn");o&&o.addEventListener("click",async()=>{const{initializeAudioCall:i}=await $(async()=>{const{initializeAudioCall:l}=await import("./calls-m9Rf1S7x.js");return{initializeAudioCall:l}},[]);i(u)}),a&&a.addEventListener("click",async()=>{const{startVideoCall:i}=await $(async()=>{const{startVideoCall:l}=await import("./calls-m9Rf1S7x.js");return{startVideoCall:l}},[]);i(u)})}}}async function ce(){const e=document.getElementById("messagesArea");if(!(!e||!u))try{console.log(" Rendu des messages pour:",u.name);const t=await ee(u.id);if(u.messages=t,e.innerHTML="",t.length===0){e.innerHTML=`
  ...
  `,s}function le(){const e=document.getElementById("messageInput");e&&(e.style.display="flex")}function st(){const e=document.getElementById("messageText"),t=document.getElementById("sendButton"),n=document.getElementById("voiceBtn");if(!e||!t)return;if(n){let r=!1;n.addEventListener("click",async()=>{if(r){const{stopVoiceRecording:o}=await $(async()=>{const{stopVoiceRecording:a}=await import("./audio-recorder-Ccx0G9hV.js");return{stopVoiceRecording:a}},[]);o(),r=!1}else{const{startVoiceRecording:o}=await $(async()=>{const{startVoiceRecording:i}=await import("./audio-recorder-Ccx0G9hV.js");return{startVoiceRecording:i}},[]);await o()&&(r=!0)}})}async function s(){if(D){console.log("⏳ Envoi déjà en cours, ignoré");return}const r=e.value.trim();if(r&&!(!u&&!L))try{D=!0;const o=b();if(!o)return;console.log("📤 Envoi message:",r);const a={id:Date.now(),senderId:o.id,receiverId:L?L.id:u.contactId,text:r,sent:!0,time:new Date().toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"}),timestamp:new Date().toISOString(),type:"text",status:"sent"};e.value="";const i=F(a),l=document.getElementById("messagesArea");l&&(l.appendChild(i),l.scrollTop=l.scrollHeight),L?await xt(o.id,L.id,a):await Me(o.id,u.contactId,a),C==="chats"&&await I(),console.log("✅ Message envoyé avec succès")}catch(o){console.error("❌ Erreur envoi message:",o),c("Erreur lors de l'envoi","error")}finally{D=!1}}t.addEventListener("click",s),e.addEventListener("keypress",r=>{r.key==="Enter"&&!r.shiftKey&&(r.preventDefault(),s())})}function rt(){N()&&(document.getElementById("sidebar").style.display="flex",document.getElementById("chatArea").style.display="none"),u=null,L=null,window.currentChat=null,window.currentGroup=null,document.getElementById("chatHeader").style.display="none",document.getElementById("messageInput").style.display="none",ue()}function ot(){const e=document.getElementById("sidebar"),t=document.getElementById("profilePanel"),n=document.getElementById("chatArea");e.style.display="none",n.style.display="none",t.style.display="flex",at()}function de(){const e=document.getElementById("sidebar"),t=document.getElementById("profilePanel"),n=document.getElementById("chatArea");t.style.display="none",e.style.display="flex",(u||L)&&(n.style.display="flex")}function at(){const e=b();if(e){const t=document.getElementById("profileImage"),n=document.getElementById("profileName");t&&(t.src=e.avatar,t.alt=e.name),n&&(n.textContent=e.name)}}function it(){const e=b(),t=document.querySelectorAll(".user-avatar img");e&&t.length>0&&t.forEach(n=>{n.src=e.avatar,n.alt=e.name})}function ue(){const e=document.getElementById("messagesArea");e&&(e.innerHTML=`
      <div class="flex items-center justify-center h-full text-gray-500">
        <div class="text-center">
    ...
    `)}function ct(){!N()&&(u||L)&&(document.getElementById("sidebar").style.display="flex",document.getElementById("chatArea").style.display="flex")}function N(){return window.innerWidth<768}function lt(){setInterval(async()=>{C==="chats"&&await I()},1e4)}function dt(e,t){const n=b();if(n){if(u&&u.id===t.id){const s=F(e),r=document.getElementById("messagesArea");r&&(r.appendChild(s),r.scrollTop=r.scrollHeight)}C==="chats"&&I(),e.senderId!==n.id&&c(` Nouveau message de ${t.name}`,"info")}}function ut(e,t){if(document.querySelectorAll(".chat-item").forEach(s=>{const r=B.find(o=>o.id===s.dataset.chatId);if(r&&r.contactId===e){const o=s.querySelector(".online-indicator");if(t&&!o){const a=s.querySelector(".relative");a.innerHTML+='<div class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-[#222e35]"></div>'}else!t&&o&&o.remove()}}),u&&u.contactId===e){const s=document.getElementById("chatStatus");s&&(s.textContent=t?"en ligne":"hors ligne")}}function mt(e){const t=new Date(e),s=new Date().getTime()-t.getTime(),r=Math.floor(s/6e4);return r<60?`${r} minutes`:r<1440?`${Math.floor(r/60)} heures`:`${Math.floor(r/1440)} jours`}function ft(){console.log("🚀 Initialisation simple des groupes..."),setTimeout(()=>{document.querySelectorAll(".filter-tab").forEach((t,n)=>{const s=t.textContent.trim().toLowerCase();if(s.includes("groupe")||s.includes("group")||n===3){console.log("📱 Onglet Groupes trouvé:",s);const r=t.cloneNode(!0);t.parentNode.replaceChild(r,t),r.addEventListener("click",me)}})},1e3)}async function me(){console.log("📱 Affichage des groupes..."),window.showSimpleGroups=me;const e=document.getElementById("chatList");if(e){document.querySelectorAll(".filter-tab").forEach(t=>{t.classList.remove("active","bg-green-600","text-white"),t.classList.add("text-gray-400")}),event.target.classList.add("active","bg-green-600","text-white"),event.target.classList.remove("text-gray-400"),C="groups";try{const t=b();if(!t){c("Vous devez être connecté","error");return}const{getUserGroups:n}=await $(async()=>{const{getUserGroups:o}=await import("./groups-DikqcN2l.js");return{getUserGroups:o}},[]),s=await n(t.id);e.innerHTML=`
      <div class="p-4">
              ...
              <div class="flex items-center space-x-3">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between">
                    ...
                    <span class="text-xs text-gray-400">${wt(o.lastMessageTime)}</span>
                  </div>
    ...
    `;const r=document.getElementById("createGroupBtn");r&&r.addEventListener("click",async()=>{try{const{createGroupModal:o}=await $(async()=>{const{createGroupModal:a}=await import("./groups-DikqcN2l.js");return{createGroupModal:a}},[]);o(a=>{a&&c(`Groupe "${a.name}" créé avec succès`,"success")})}catch(o){console.error("Erreur chargement module groupes:",o),c("Erreur lors du chargement du module groupes","error")}}),document.querySelectorAll(".group-item").forEach(o=>{o.addEventListener("click",()=>{const a=o.dataset.groupId,i=s.find(l=>l.id===a);i&&gt(i)})}),console.log(`✅ ${s.length} groupe(s) affiché(s)`)}catch(t){console.error("❌ Erreur affichage groupes:",t),e.innerHTML=`
      <div class="text-center py-8 text-red-400">
        <i class="fas fa-exclamation-triangle text-4xl mb-4"></i>
    ...
    `}}}async function gt(e){try{console.log("💬 Ouverture du chat de groupe:",e.name),u=null,window.currentChat=null,L=e,window.currentGroup=e,N()&&(document.getElementById("sidebar").style.display="none"),document.getElementById("chatArea").style.display="flex",pt(e),await yt(e),le(),c(`Groupe "${e.name}" ouvert`,"success")}catch(t){console.error("❌ Erreur ouverture groupe:",t),c("Erreur lors de l'ouverture du groupe","error")}}function pt(e){const t=document.getElementById("chatHeader"),n=document.getElementById("chatAvatar"),s=document.getElementById("chatName"),r=document.getElementById("chatStatus");if(t&&e){t.style.display="flex",n.innerHTML=`
      <div class="relative">
    ...
    `,s.textContent=e.name,ht(e,r);const o=document.getElementById("callButtons");if(o){const a=b(),i=e.admins&&e.admins.includes(a.id);o.innerHTML=`
        <div class="relative">
      ...
      `;return}s.forEach(r=>{const o=vt(r,e);t.appendChild(o)}),t.scrollTop=t.scrollHeight,console.log(`✅ ${s.length} messages de groupe affichés`)}catch(n){console.error("❌ Erreur rendu messages groupe:",n),t.innerHTML=`
  ...
  `,r}async function xt(e,t,n){try{console.log("📤 === ENVOI MESSAGE GROUPE ==="),console.log("Expéditeur:",e),console.log("Groupe:",t),console.log("Message:",n.text);const{sendMessageToGroup:s}=await $(async()=>{const{sendMessageToGroup:o}=await import("./groups-DikqcN2l.js");return{sendMessageToGroup:o}},[]),r=await s(e,t,n);return console.log("✅ Message de groupe envoyé:",r),r}catch(s){throw console.error("❌ Erreur envoi message groupe:",s),s}}function wt(e){if(!e)return"";const t=new Date(e),s=new Date-t;return s<24*60*60*1e3?t.toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"}):s<7*24*60*60*1e3?t.toLocaleDateString("fr-FR",{weekday:"short"}):t.toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit"})}document.addEventListener("DOMContentLoaded",()=>{ft()});export{c as s};
